...
const getCookieInAdmin = (name: string): string | null => {
...
export default function AdminDashboardPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [srvRes, reqRes, logRes] = await Promise.all([
        apiFetch("/servers"),
        apiFetch("/requests"),
        apiFetch("/logs"),
      ]);
  ...
  const handleRevoke = async (id: string) => {
    if (!confirm("Are you sure you want to revoke this access immediately?"))
      return;
    try {
      const res = await apiFetch(`/requests/${id}/revoke`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      if (res.ok) {
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Management and observability for JIT infrastructure access.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="bg-card/60 backdrop-blur-sm border-border hover:border-primary/50 transition-colors relative overflow-hidden">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0 relative z-10">
            ...
            <ShieldAlert
              className={`w-4 h-4 ${pendingRequests.length > 0 ? "text-amber-500" : "text-muted-foreground"}`}
            />
          </CardHeader>
          <CardContent className="relative z-10">
            <div
              className={`text-3xl font-bold ${pendingRequests.length > 0 ? "text-amber-500" : "text-foreground"}`}
            >
            ...
            <div className="space-y-4">
              {pendingRequests.length === 0 ? (
                ...
                pendingRequests.map((req) => (
                  <div
                    key={req.id}
                    className="flex items-center justify-between p-4 border rounded-xl hover:bg-muted/30 transition-colors bg-background/50"
                  >
                      ...
                      <div>
                        <h4 className="font-semibold text-sm flex items-center gap-2">
                          ...
                          <span className="text-primary">
                            {req.server?.hostname || req.server_id}
                          </span>
                        </h4>
                    ...
                    <div className="flex flex-col gap-2 items-end">
                      ...
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-8 gap-1 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/10 border-emerald-500/20"
                          onClick={() => {
                            const dur = (
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-8 gap-1 text-destructive hover:bg-destructive/10 border-destructive/20"
                ...
                {requests.filter(
                  ...
                  requests
                    ...
                    .map((req) => (
                      <div
                        key={req.id}
                        className="p-3 border rounded-lg bg-background/40 flex justify-between items-center"
                      >
                        <div>
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-2 h-2 rounded-full ${srv.status === "online" ? "bg-emerald-500 animate-pulse" : "bg-muted-foreground"}`}
                          ></div>
