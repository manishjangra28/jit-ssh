...
export default function AgentTokensPage() {
  ...
  const fetchTokens = async () => {
    try {
      setLoading(true);
      const res = await apiFetch("/agent-tokens");
      if (res.ok) {
  ...
  const handleRevoke = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to revoke this token? Any agent using it will lose access immediately.",
      )
    )
      return;

    try {
      const res = await apiFetch(`/agent-tokens/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        fetchTokens();
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-card/60 p-6 rounded-xl border border-border backdrop-blur-sm">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Generate pre-shared secrets to secure communication between your
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            className="gap-2"
            onClick={() => window.open(`${getApiUrl()}/agent/deploy/download`)}
          >
            <Download className="w-4 h-4" /> Download Latest Agent
          </Button>
        ...
        <Card className="border-blue-500/50 bg-blue-500/5 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2 text-blue-400">
              <ShieldAlert className="w-5 h-5" /> New Token Generated:{" "}
              {revealedToken.label}
            </CardTitle>
            <CardDescription className="text-blue-400/80">
              Copy this token now. It will NOT be shown again for security
              reasons.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-card/60 border-border backdrop-blur-sm">
          <form onSubmit={handleCreate}>
            <CardHeader>
              ...
              <CardDescription>
                Give this token a descriptive label to identify the agent or
                group of agents using it.
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      <Card className="bg-card/60 border-border backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription>
            Currently valid tokens that allow agents to connect.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 border-2 border-dashed rounded-xl border-border">
              ...
              <p className="text-sm text-muted-foreground">
                Generate a token to start connecting agents.
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {tokens.map((token) => (
                    <tr
                      key={token.id}
                      className="border-b border-border bg-background/50 hover:bg-muted/50 transition-colors"
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-1">
                          <Button
                            ...
                            onClick={() =>
                              window.open(
                                `${getApiUrl()}/agent/deploy/script?token_id=${token.id}`,
                              )
                            }
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="bg-blue-500/5 border border-blue-500/20 rounded-lg p-4 flex gap-3">
          ...
          <div className="text-sm text-blue-200/80">
            ...
            <p>
              Click the <Terminal className="w-3 h-3 inline" /> icon next to a
              token in the table above to download a pre-configured bash script.
              Run it as root on any server to instantly install and register the
              JIT Agent.
            </p>
          </div>
        </div>
        <div className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-4 flex gap-3">
          ...
          <div className="text-sm text-amber-200/80">
            ...
            <p>
              Always use unique tokens per environment or cluster. If an agent
              machine is compromised, revoke its token immediately to block
              access to the JIT control plane.
            </p>
          </div>
        </div>
      </div>
